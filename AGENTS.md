...
- Создавать fallback-и с дефолтными данными нужно создавать только тогда, когда это действительнно необходимо
...
- Все обращения к внешнему API должно происходить через слой data, а обращение к data-слою через Mobx за исключением случаев изначальной загрузки по SSR
...
- Не унифицированные стили, относящиеся к компоненту давай описывать перед самим компонентом в файле компонента
